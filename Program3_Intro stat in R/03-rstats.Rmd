...
**Hypothesis that we are interested with this example**: whether the proportion of patients who have history of colon polyps in the patients with colon cancer is different from that
...
The null hypothesis is the independence of rows and columns in a contingency table with fixed marginals. It compares the distributions of counts within 4 cells. The null hypothesis is specified in terms of odd ratio=1. It produces confidence interval for the odd ratio. The Fisher's exact test can be used with small sample sizes.
...
z.prop.p(cancer.polyps[1,1], cancer.polyps[2,1], sum(cancer.polyps[1,]), sum(cancer.polyps[2,]))
```
...
z.prop.ci(cancer.polyps[1,1], cancer.polyps[2,1], sum(cancer.polyps[1,]), sum(cancer.polyps[2,]))
```
