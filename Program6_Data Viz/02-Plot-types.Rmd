...
```{r}
g + geom_text(aes(label = Sub_group))
```

`check_overlap` can remove the overlapped texts
```{r}
g + geom_text(aes(label = Sub_group), check_overlap = TRUE)
```
...
```{r}
g + geom_text(aes(label = Sub_group), nudge_y = 0.05, check_overlap = TRUE) +
  geom_point()
```
...
```{r}
ggplot(CRC) + 
  geom_boxplot(aes(Sub_group, AFM))
ggplot(CRC) + 
  geom_boxplot(aes(Sub_group, AFM, fill= factor(Gender)), position = "dodge")
```